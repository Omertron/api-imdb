...
//        List<TextLabel> result = ImdbApi.getParentalGuide(IMDB_ID);
//        for(TextLabel tl : result) {
...
//        List<UserComment> result = ImdbApi.getUserReviews(IMDB_ID);
...
//        List<Review> result = ImdbApi.getExternalReviews(IMDB_ID);
...
//        List<ComingSoonList> result = ImdbApi.getComingSoon();
//
...
//        ImdbActor result = ImdbApi.getActorDetails(ACTOR_ID);
...
//    }
    ...
    public void testGetSearch() {
        ...
        Map<String, List<ImdbMovieDetails>> result = ImdbApi.getSearch("Avengers");

        for(Entry<String, List<ImdbMovieDetails>> entry : result.entrySet()) {
            LOGGER.info("Type: " + entry.getKey() + " contains " + entry.getValue().size() + " entries");
        }
