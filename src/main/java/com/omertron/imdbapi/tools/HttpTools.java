...
public class HttpTools {
     ...
     */
    public String getRequest(final URL url) throws ImdbException {
        try {
            ...
            throw new ImdbException(ApiExceptionType.CONNECTION_ERROR, null, url, ex);
        } catch (RuntimeException ex) {
            throw new ImdbException(ApiExceptionType.HTTP_503_ERROR, "Service Unavailable", url, ex);
        }
    }
     ...
     * @throws ImdbException
     */
    public String deleteRequest(final URL url) throws ImdbException {
        try {
     ...
     * @throws ImdbException
     */
    public String postRequest(final URL url, final String jsonBody) throws ImdbException {
        try {
     ...
     * @throws ImdbException
     */
    private String validateResponse(final DigestedResponse response, final URL url) throws ImdbException {
        if (response.getStatusCode() == 0) {
            throw new ImdbException(ApiExceptionType.CONNECTION_ERROR, response.getContent(), response.getStatusCode(), url, null);
        } else if (response.getStatusCode() >= HttpStatus.SC_INTERNAL_SERVER_ERROR) {
            throw new ImdbException(ApiExceptionType.HTTP_503_ERROR, response.getContent(), response.getStatusCode(), url, null);
        } else if (response.getStatusCode() >= HttpStatus.SC_MULTIPLE_CHOICES) {
            throw new ImdbException(ApiExceptionType.HTTP_404_ERROR, response.getContent(), response.getStatusCode(), url, null);
        }
